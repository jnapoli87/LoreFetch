...
Owns (exclusive write access): `LoreFetch.Core/Identification/**`, `LoreFetch.Core/Imaging/**`, `LoreFetch.Lab/**`
Consumes: `Core/Abstractions` (frozen — see [`CONTRACTS.md`](CONTRACTS.md)), the local fixture corpus
Must not touch: `LoreFetch.App`, `LoreFetch.Capture`, any `.csproj`, `LoreFetch.slnx`
...
Over the fixture corpus, report **correct@1 / wrong@1 / no-match** per camera height, per difficulty rung, and the **distance-margin distribution** (true match vs best impostor). The margin is what sets `GoodDistance` and `OkDistance`.
...
- `GoodDistance` and `OkDistance` committed as measured values with the margin data behind them.
...
7. **Does anything here hardcode a threshold** that should come from `ScanSettings`?
