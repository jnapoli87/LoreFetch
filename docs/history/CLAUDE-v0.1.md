# CLAUDE.md as it stood before the trim

> [!IMPORTANT]
> **Historical, not instructions.** This is the full `CLAUDE.md` from the end of the post-v0.1 cleanup, just before it was split up. Its settled decisions now live in [`../DECISIONS.md`](../DECISIONS.md), the working rules in [`../../CLAUDE.md`](../../CLAUDE.md), and this copy keeps the rest: the guard-hook war stories and the reasoning trail. Its relative links are as they were at the repo root, so some do not resolve from here; the original is at tag `v0.1.0` and in git history.
