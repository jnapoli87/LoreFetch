...
### Ruling (user, 2026-09-22, integration session): **15″ only, nothing else — not enough time**

**Supersedes the 12″ operating height and the 12″/20″ sweep** in the geometry re-ruling, H3's shot list and the B-closing handoff's E1 notes. The operating height is **~15″**, and it is the only height measured. At 15″ a **portrait** 3×3 fits with +1.21″ of margin, so **cards are not rotated** (H3's portrait shot list stands). The expected card width is `1360 × 2.5 / 15` ≈ **227 px**. `CameraRotationDegrees = 0` at composition is unchanged. E1 becomes a single-height calibration at 15″. Because `goodDistance`/`okDistance` (208/240) were set on 15″ frames, E1's job is to test them on **fresh cards** under the same condition.

Revised H3 shot list (all 15″): 3×3 light, 3–4 fresh groups · 3×3 dark, 1–2 of those groups · 3×3 light with ~2 mm gaps, 1 · 3-line light, 1–2 · single light, 1–2. If time runs short, drop the 3-line and single rows before the dark-mat and tight-gap rows.

### Handoff — end of the integration session, 2026-09-22 night, Windows PC — **hackathon over**

**Newest handoff; read this first.** Integration I1–I6 is done, DH landed, and **P6 was pushed** (`16a2666`) with the user's approval. The user ended the hackathon here. Everything below is "later".

- **CI is red on GitHub** (#28 `a687a44` and #29 `16a2666`: both legs exit 1). #28 predates this session. **A clean-clone run of `scripts/lorefetch.sh test --no-pull` on the Windows PC, with no imagery and the cache pointed at a nonexistent path, passes:** Integration 161/11 skipped, StreamA 151, StreamB 310/9, StreamC 39, StreamD 71. So the failure is specific to the CI environment. The logs need a signed-in viewer. The user will look.
- **Release exe: built, zipped and smoke-tested but NOT published.** `C:\LoreFetchData\release-staging\LoreFetch-v0.1.0-win-x64.zip` (86 MB) holds `LoreFetch.exe` (single-file, self-contained, 193 MB, no loose native DLLs), `data/index/{cards.lfidx,thresholds.json}`, README, LICENSE and THIRD-PARTY-NOTICES. Extracted into a fresh folder, it loaded the index and ran the real pipeline, exit 0. It was not tried on a clean Windows profile without .NET (E2's last check). It is unsigned, so SmartScreen will warn. Built from `16a2666` with `dotnet publish src/LoreFetch.App -c Release -r win-x64 --self-contained -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true` (10 s). **P7** (tag `v0.1.0` and the GitHub release) needs the user's approval. Create the release in the browser, never via `gh` (it is bound to the work account and not installed on the PC).
- **Open, in priority order:** E1 live pass 2 on the DH build (black mat, tight grid; the build is at `C:\LoreFetchData\live-run-2`); the README needs a "download and run" section for the exe; CI; the performance question (3.4 ms/query against CLAUDE.md's 0.243 ms; 63 ms per dual cohort); E3 video.
- **Learned:** (1) **A blocking wait on the UI thread is safe with fakes and deadlocks with COM.** Every headless and Fakes-mode test passed while the real camera open hung, and only a live launch on real hardware found it. (2) **Measure before deferring.** The user nearly shelved DH as an unproven optimisation; reverting it made two confident wrong matches (236, 239) reappear in the shipped configuration, which was the evidence that settled it.

### Ruling (user, 2026-09-22, integration session): **dual-hypothesis identification ships in v0.1.0**

**Found by E1a on the user's 15″ corpus** (`test-images/integration_corpus/`, 11 frames, 57 ground-truth rows appended). On touching cards (`tight_white`) and on a black mat, the detector's quad lands on the **inner** edge of the black border, because adjacent borders merge and a black border on a black mat has no contrast. The rectified crop is clean but borderless, while every index render carries its border. Result: every card at ~290–350 (noise), which is B5c's crop-scale failure. It also cost 3 cards on the normally spaced `z_` frames. **Measured border ratio** (40 Scryfall renders): width ×1.085, height ×1.089.

Experiment (`lab expand-experiment`, `4154a6b`, re-run by the orchestrator): non-land correct@1 **baseline 22/38 · expanded-only 17/38 · dual (minimum distance of the two) 37/38**. tight_white 0/6 → 6/6; black mat 0/6 → 6/6; **0 regressions** (baseline-correct → dual-wrong); **0 wrong matches ≤ 240** under either hypothesis. The one dual miss (Seven-Tail Mentor, 246) stays unresolved rather than going confidently wrong. Expanded-only is a net regression, so **both hypotheses are needed**. This reopens B5c's "sweep rejected for v1" on new evidence; the cost is 2 identifies per card, not a sweep.

**Not fixed by it, so document it:** 3 cards touching in a line merge into one contour (`x_1` 0/3, `x_2` 1/3). README: "leave a finger-width gap."

**Also recorded:** the frames' derived height is 15.3–15.7″ (and 17.0–17.2″ on the inset frames, which is the inset itself showing through the formula). The camera also held 1080p MJPG @30 fps live, per I3's hotfix note.
...
| **I6** 🧭👤 | `LOREFETCH_REQUIRE_REAL=1 dotnet test` on this PC, **0 skipped**, then **P6**. | **Prerequisites: all three are done on this PC as of 2026-09-22.** (a) The cache path: the tests now fall back to `C:\LoreFetchData\scryfall-cache` on Windows when `~/LoreFetchData/scryfall-cache` is absent (`73e2c80`), and the user environment variable `LOREFETCH_SCRYFALL_CACHE` is also set. The Mac keeps using its home-folder cache. (b) `test-images/fixtures/15in/9/a_1..a_6.png` are in the main checkout (copies of `a_corpus`). (c) `test-images/ground-truth.csv` is in the main checkout. Neither is ever committed. With all three, `main` runs StreamB **307 passed / 1 skipped** without the variable. B's perf test soft-skips under CPU load, so read it from an isolated run. |
...
- [x] **I1** After D merges: implement `Real` in `Tests/Integration` for the store and exporters, and switch `AppComposition` to use them. — *done 2026-09-22, `0c531dd` (Sonnet, main checkout, orchestrator-verified). `CompositionMode.Real` via `LOREFETCH_MODE=fakes|real` (default still Fakes until I3; an unknown value logs and falls back). Collection file: `<MyDocuments>/LoreFetch/collection.csv`, overridable with `LOREFETCH_COLLECTION` (orchestrator ruling). The WindowsOnly integration test locks `collection.csv` mid-commit through `ComposeAsync`: `CollectionStoreException` surfaces, the cohort is retained, and a retry after release lands the rows. Both regression tests were chaos-tested. Suites: build 0/0; Integration **161 passed / 8 skipped** (the 8 Real skips are all detector/identifier/frame-source, I2's); StreamA **147**; StreamD 71.*
- [x] **I2** After A and B merge: `Real` gets the detector, rectifier, identifier and catalog, loaded from `DataFiles`. B7's synthetic frames take the `FolderFrameSource` slot. `AppComposition` gets a `Real` mode that loads the index and thresholds. — *done 2026-09-22, `faa865d` (Sonnet, main checkout, combined with I3). Real mode: `ContourCardDetector` (List), `PerspectiveRectifier`, one `HashCardIdentifier.Load` as identifier **and** catalog (47,418 entries / 32,743 oracle ids), thresholds 208/240 from `thresholds.json` with no literals. `LOREFETCH_FRAMES_DIR` in Real mode replays a folder through the real pipeline. A missing index exits 1 with a critical log naming the path, and never falls back to Fakes. `RealImplementationSet` fills every slot from synthetic 15″ frames rendered from real Scryfall images. Integration **169 passed / 0 skipped**, and the same with `LOREFETCH_REQUIRE_REAL=1`. Chaos-tested: hardcoded thresholds, rotation 90, fallback to Fakes.*
- [x] **I3** After C merges: `WebcamFrameSourceFactory` goes into `AppComposition`, with a switch between the demo folder and the camera. — *done 2026-09-22, `faa865d` + orchestrator hotfix `64c767d`. `WebcamFrameSourceFactory` by default, `CameraRotationDegrees = 0`, and **`LOREFETCH_MODE` now defaults to `real`**. No camera present → the `SourceFailed` path, not a crash. **Hotfix:** the live camera launch hung after device enumeration, because `App` blocked the STA UI thread on composition while FlashCap's DirectShow (COM) open needed that thread's message pump. Composition now runs on a pool thread. Verified live: C920 1920×1080 MJPG @30 fps over DirectShow, first frame 705 ms, pipeline ~29 fps, preview ~15 fps, decode ~7.8 ms/frame. There is no automated test for the deadlock; the live launch is the acceptance.*
- [x] **I4** 🧭 Grep for any hardcoded distance outside `Core/Scanning`, `CohortTile` and `thresholds.json`. There must be none. **Exempt:** `src/LoreFetch.App/Fakes/DemoThresholds.cs`, Fakes-mode demo values (ruled at A10-prep). — *done 2026-09-22 (orchestrator grep). No distance literal in App or Core outside the allowed places. Remaining hits are rotation angles, colours, Core/Fakes stub identifier **outputs** (not thresholds), and the Lab accuracy harness's `270`, which is a documented fallback used only when `thresholds.json` cannot load, announced on stderr, and in a dev tool that doesn't ship.*
- [x] **I5** `THIRD-PARTY-NOTICES`: an entry for every package, so the hook's advisory list is empty. Chase down the FFmpeg notices in the OpenCvSharp runtimes (PLAN risk 7). — *done 2026-09-22, `53c9f6b`. Every shipped package, transitive natives included (Skia, HarfBuzz, ANGLE), with copyright holder and licence text. The advisory list is empty. **FFmpeg in the OpenCvSharp runtimes is LGPL-2.1, not GPL:** the embedded configure strings show no `--enable-gpl`/`--enable-nonfree`. That is binary-string evidence only, because the package ships no FFmpeg licence file. The LGPL relinking obligation is met by LoreFetch being GPLv3 source that anyone can rebuild; this goes on the release checklist.*
- [x] **I6** 🧭👤 `LOREFETCH_REQUIRE_REAL=1 dotnet test` on the **Windows PC** with every artifact present gives **0 skipped**. Then P6. — *done 2026-09-22 on the Windows PC, `aa97989`, `LOREFETCH_REQUIRE_REAL=1`: Integration **172 passed / 0 skipped**; StreamA 151; StreamD 71; StreamC 39 (`Category!=Hardware`); StreamB **317 passed / 2 skipped / 0 failed**. The 2 are the soft-budget performance tests, which are not artifact-gated. Isolated: 9 identify calls 30.5 ms (3.4 ms/query, one hypothesis); dual 18 calls **63.3 ms** per 9-card cohort, over the 50 ms soft budget. This is ~14× CLAUDE.md's 0.243 ms/query brute-force figure and predates DH (the single-hypothesis path is 3.4 ms too). It runs once per capture, not per frame, so it is not a v0.1.0 blocker; it is a v0.2 investigation. Also landed in this stretch: **DH** `6dcdeee` (dual-hypothesis in `ScanPipeline` via `Core/Scanning/DualHypothesisIdentification`, geometry in `Core/Imaging/QuadExpansion`; full 111-slot corpus correct@1 **60% → 84%**, confident wrong@1 **2 → 0**; StreamB's accuracy gate passes with no threshold changed; 4 chaos cases), and the Fakes demo fix `aa97989`.*
