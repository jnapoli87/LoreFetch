...
- **Release exe: built, zipped and smoke-tested but NOT published.** `C:\LoreFetchDataelease-staging\LoreFetch-v0.1.0-win-x64.zip` (86 MB) holds `LoreFetch.exe` (single-file, self-contained, 193 MB, no loose native DLLs), `data/index/{cards.lfidx,thresholds.json}`, README, LICENSE and THIRD-PARTY-NOTICES. Extracted into a fresh folder, it loaded the index and ran the real pipeline, exit 0. It was not tried on a clean Windows profile without .NET (E2's last check). It is unsigned, so SmartScreen will warn. Built from `16a2666` with `dotnet publish src/LoreFetch.App -c Release -r win-x64 --self-contained -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true` (10 s). **P7** (tag `v0.1.0` and the GitHub release) needs the user's approval. Create the release in the browser, never via `gh` (it is bound to the work account and not installed on the PC).
