...
5. **`net8.0` or `net10.0`.** **Decision: `net10.0` — the user's call, overriding my `net8.0`.** Stream A's own stated reason for `net8.0` ("wider compatibility") does not hold anyway: a self-contained single-file publish bundles the runtime, so the user's machine never sees the TFM. That left only "the pins were verified against `net8.0`", against `net10.0`'s longer support life — and `net8.0` leaves LTS in Nov 2026.
   ...
   **The risk this takes on, and where it is paid.** Smaller than I first stated: stream A read Avalonia 12.1.2's nuspec and found it targets **`net8.0` and `net10.0` only**, so the largest and most constraining pin is explicitly verified against `net10.0`. The rest ship `netstandard2.0`/`net8.0` (`CommunityToolkit.Mvvm`, OpenCvSharp, FlashCap) which a `net10.0` project consumes normally, and `TextFieldParser` rides in the shared framework. So the residual risk is ordinary restore risk, not a known incompatibility.

   It still has to be *proven* rather than assumed, because `.csproj` files are hook-enforced frozen from the fork onward and discovering a bad pin afterwards stalls all four streams. Stream 0 item 1 therefore gates on a clean restore **and** build of every project on `net10.0`, falling back to `net8.0` there if any pin fails.
