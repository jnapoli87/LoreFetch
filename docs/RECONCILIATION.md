...
> [!IMPORTANT]
> Six decisions are cheap to change now and expensive or impossible after Stream 0 freezes. They are listed under [Awaiting your call](#awaiting-your-call) and are **applied** — nothing is blocked on them — but say the word and they flip.
...
5. **`net8.0` or `net10.0`.** **Decision: `net8.0`** — every pinned package is verified against it, Avalonia 12 dropped everything below it, and nothing needs `net10.0`. Note that A's stated reason ("wider compatibility") does not hold: a self-contained single-file publish bundles the runtime, so the user's machine never sees the TFM. The decision stands on verified-pins grounds instead. → [Awaiting your call](#awaiting-your-call).
...
4. **Duplicate `OracleId` + `Condition` rows on read?** **Decision: merge — sum `Quantity`, keep the latest `LastScannedAt` — and log it.** A deliberate exception to the fail-loudly rule, which governs *columns*. Hand-editing in Excel is an advertised feature, so a pasted or re-sorted row must not lock a user out of their own collection; the file is the source of truth and the app should heal what it safely can. D flagged this as needing a human rather than its own call, which was right. → [Awaiting your call](#awaiting-your-call).
...
All six are **applied** — nothing is blocked. They are here because each is cheap to reverse today and expensive or impossible once Stream 0 freezes.

| # | Decision | Why you might want the other one |
|---|---|---|
| 1 | **The gate is a measured distance floor, not ≈ 0**, and the transforms are asymmetric by design (B-Q1) | This rewrites *the one gate that matters most* — the project's stated highest-risk guard. B's evidence is strong and the old text contradicted itself, but it is your invariant. |
| 2 | **Pull `normal` images: ~6.0 GB, not ~0.79 GB** (B-Q2) | It is your disk and roughly an hour of your wall clock. `border_crop` (480×680) is a near-identical fallback. Never `small`. |
| 3 | **Duplicate rows merge on read rather than failing loudly** (D-Q4) | A deliberate exception to a rule you set. The alternative locks a user out of their collection after an innocent Excel paste. |
| 4 | **Index built and committed on `win-x64` only** | Consequence: the Mac cannot regenerate a matching index, because `INTER_AREA` is not bit-exact across architectures. Development on the Mac is unaffected; only index *generation* is pinned. |
| 5 | **`net8.0`** (A-Q5) | Verified against every pin, but `net8.0` leaves LTS support in Nov 2026. `net10.0` is the longer-lived choice if you would rather carry the compatibility risk. |
| 6 | **`AvaloniaUI.DiagnosticsSupport` pinned** (A-Q1) | Primary sources genuinely conflict on whether Avalonia 12's DevTools needs a paid tier. Pinned costs nothing; if it is paid, stream A ships without DevTools. |
...
| [`PLAN.md`](PLAN.md) | Review steps 2 and 3 marked done. Stream 0: the reconciled package table, `net8.0`, seven fakes, pipeline factory, thresholds loader; estimate ~3.5h → ~4h. |
