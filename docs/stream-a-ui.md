...
- **Staying on 12.1.x is a hard pin in its own right.** (This finding originally hung off `OpenCvSharp5.AvaloniaExtensions`, which pinned Avalonia 12.1.1; that package is dropped — V4 — and the pin stands without it.) Avalonia 12.0.x lost 5–40× render-pass CPU to region dirty-rect clipping (≈70 ms/frame at 1440p) — fixed in 12.1.x, which defaults to the safe path — https://github.com/AvaloniaUI/Avalonia/issues/21538#issuecomment-4679543248
