# LoreFetch.Tests.Collection

Unit tests for `Core/Collection` and `Core/Export` — the CSV collection store and export adapters (native and Moxfield). The Collection and export domain's test project; see the [domain map](../../docs/CONTRACTS.md#domain-map).
...
# or directly:
dotnet test Tests/Collection/LoreFetch.Tests.Collection.csproj -c Release
```
