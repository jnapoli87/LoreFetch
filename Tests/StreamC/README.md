...
dotnet test Tests/StreamC/LoreFetch.Tests.StreamC.csproj -c Release --filter "Category!=Hardware"
```
...
```sh
dotnet test Tests/StreamC/LoreFetch.Tests.StreamC.csproj -c Release --filter "Category=Hardware&Interactive!=Unplug"
```

Interactive (waits for a person to physically unplug the camera on cue):

```sh
dotnet test Tests/StreamC/LoreFetch.Tests.StreamC.csproj -c Release --filter "FullyQualifiedName~UnplugCameraTests.UnplugProducesCleanErrorWithinWatchdog"
```
