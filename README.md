...
| **Special hardware.** | A webcam looking straight down at the table: the same kind of overhead setup people already use for SpellTable. The reference rig is a hand-built PVC gantry, cut with pipe cutters from the [Buffalo Tool Library](https://www.buffalotoollibrary.org), with the webcam duct-taped to the crossbar. |
