...
Real-capture regression tests additionally need `test-images/` (gitignored, never committed — see [`../../docs/DECISIONS.md`](../../docs/DECISIONS.md#card-imagery-is-enforced-in-two-layers-not-just-documented)): `test-images/ad-hoc/` for individual detector fixtures, `test-images/fixtures/<height>in/<layout>/` plus `test-images/ground-truth.csv` for the accuracy harness. Those also skip, not fail, when absent.
